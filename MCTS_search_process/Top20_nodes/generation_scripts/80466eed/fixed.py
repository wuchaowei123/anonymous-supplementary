...
Type: fixed
Generated: 2026-01-12T22:16:40.684071
Fix Attempts: 1
...
class UserSeqDataset(Dataset):
    ...
    def __getitem__(self, idx):
        xnum = torch.from_numpy(self.X_num[idx])
        xagg = torch.from_numpy(self.X_agg[idx])
        if self.X_cat is None:
            xcat = None
        else:
            xcat = torch.from_numpy(self.X_cat[idx])
...
class EulerResNetEncoder(nn.Module):
    def __init__(self,
        ...
        for card, ed in zip(cat_cardinalities, cat_emb_dims):
            self.cat_embs.append(nn.Embedding(card + 1, ed))
            total_cat_emb += ed

        self.day_emb = nn.Embedding(max_days + 1, day_emb_dim)
    ...
    def forward(self, x_num, x_cat, x_agg):
        ...
        day_idx = torch.arange(1, T + 1, device=device).unsqueeze(0).expand(B, T)
        day_e = self.day_emb(day_idx)
...
def main():
    ...
    result = {
        ...
        "score": float(score),
        "metrics": metrics,
        "val_calibration_metrics": {k: float(v) for k, v in best_cal_metrics.items()},
    ...
    print(f"score = {score}")
    # === SAVE MODEL ===
    node_dir = "/home/jupyter/anonymous_institutionAgentMonty_1226/tasks/BT_IOS_2503_Pareto/run_20260112_102800/80466eed"
    ...
    try:
        ...
        import joblib
        save_dir = Path("/home/jupyter/saved_models/80466eed")
        save_dir.mkdir(parents=True, exist_ok=True)
        saved = []
        
        # Save all LightGBM models
        try:
            # import lightgbm as lgb (removed)
            for name in dir():
                obj = eval(name)
        ...
        with open(save_dir / "metadata.json", 'w') as f:
            json.dump({'node_id': '80466eed', 'saved': saved}, f)
...
# ========== CORRECTED SAVING CODE ==========
    ...
    save_dir = Path(f"/home/jupyter/saved_models_final/80466eed")
    save_dir.mkdir(parents=True, exist_ok=True)
