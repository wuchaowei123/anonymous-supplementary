#!/usr/bin/env python3
"""
anonymous_institutionAgentMonty - Execution Node: 59394c83
Type: fixed
Generated: 2026-01-13T13:13:45.733524
Fix Attempts: 1
...
def make_aligned_sequences(df_daily, user_ids, num_cols, cat_cols, cat_maps, scaler, add_time_features=True):
    user_ids_unique = pd.Series(user_ids).drop_duplicates().values
    idx = pd.Index(user_ids_unique)
    df = df_daily.copy()
    ...
    if add_time_features:
        t = (day.astype(np.float32) / 6.0) * 2.0 - 1.0
    ...
    Xc = encode_cats(df, cat_cols, cat_maps)
    ...
    id_to_index = {uid: i for i, uid in enumerate(user_ids_unique)}
    result_indices = np.array([id_to_index[uid] for uid in user_ids], dtype=np.int64)
    
    return X_num[result_indices], X_cat[result_indices]
...
class RK2TimeStepperNet(nn.Module):
    def __init__(self, n_num, cat_cardinalities, d_model=192, d_state=192, f_hidden=256, dropout=0.10):
        ...
        self.head_sign = nn.Linear(z_dim, 3)
        self.head_pos  = nn.Linear(z_dim, 1)
        self.head_neg  = nn.Linear(z_dim, 1)
        self.head_q20  = nn.Linear(z_dim, 1)
        self.head_q80  = nn.Linear(z_dim, 1)
...
def main():
    ...
    cat_cardinalities = [max(cat_maps[c].values()) for c in daily_cat_cols]
    ...
    out_path = "/home/jupyter/anonymous_institutionAgentMonty_1226/tasks/BT_IOS_2503_Pareto/run_deepresearch/physics_rk2_hybrid_results.json"
    try:
    ...
    print(f"score = {score}")
    # === SAVE MODEL ===
    node_dir = "/home/jupyter/anonymous_institutionAgentMonty_1226/tasks/BT_IOS_2503_Pareto/run_20260112_102800/59394c83"
    ...
    try:
        ...
        import joblib
        save_dir = Path("/home/jupyter/saved_models/59394c83")
        save_dir.mkdir(parents=True, exist_ok=True)
        saved = []
        
        # Save all LightGBM models
        try:
            # import lightgbm as lgb (removed - already imported globally)
            for name in dir():
                obj = eval(name)
        ...
        with open(save_dir / "metadata.json", 'w') as f:
            json.dump({'node_id': '59394c83', 'saved': saved}, f)
...
# ========== CORRECTED SAVING CODE ==========
    ...
    save_dir = Path(f"/home/jupyter/saved_models_final/59394c83")
    save_dir.mkdir(parents=True, exist_ok=True)
