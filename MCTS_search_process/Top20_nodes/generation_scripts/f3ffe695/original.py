#!/usr/bin/env python3
"""
AviaAgentMonty - Execution Node: f3ffe695
Type: original
Generated: 2026-01-13T20:14:09.222771
Mutation: None
Parent: None
...
# =============================================================================
# !!! SCORING FUNCTIONS - DO NOT MODIFY (PRESERVED EXACTLY) !!!
# =============================================================================
...
def _make_user_level_base(df: pd.DataFrame, numeric_cols, cat_cols, with_target: bool, max_days: int = 7):
    """
    Builds:
      - numeric aggregate features for ALL numeric cols
      - compact temporal (day-wise + diff + slog + slope) for selected seq_cols
      - user-level categorical (last value + nunique) for TE/frequency
    """
    ...
    # --- groupby handle (sorted => first/last meaningful)
    g = df.groupby(ID_COL, sort=False)

    # --- numeric aggregates (small enough, strong for GBDT)
    ...
    # add last-first diff for all numeric (helpful temporal signal with low cost)
    ...
    # also guard std NaNs -> 0
    X_num_agg = num_agg.to_numpy(np.float32)
    X_num = np.concatenate([X_num_agg, num_diff], axis=1).astype(np.float32)

    # --- choose seq cols: fixed known set + top-variance from TRAIN only
    # (caller can override by passing precomputed seq_cols if desired; here we compute per-df)
    # We'll finalize in outer pipeline using TRAIN to pick seq_cols, then reuse for val/test.
    # So here we only return user_ids, X_num, plus cat frames; temporal added later.
    cat_last = None
    cat_nuniq = None
    if cat_cols:
...
def _make_seq_features(df: pd.DataFrame, user_ids: np.ndarray, seq_cols, max_days: int = 7):
    ...
    df = ensure_day_column(df)
    df = df.sort_values([ID_COL, "TDATE_RN"])
    base = df.set_index([ID_COL, "TDATE_RN"])[seq_cols]
    ...
    # slope per feature (vectorized)
    x = np.arange(1, max_days + 1, dtype=np.float32)
    x = x - x.mean()
    denom = float((x ** 2).sum())
    y_center = arr - arr.mean(axis=1, keepdims=True)
    slope = (y_center * x[None, :, None]).sum(axis=1) / max(denom, 1e-6)  # (N, F)

    # flatten in (feature-major, then day) order => stable across splits
...
def _build_oof_target_encoding(train_cat_last: pd.DataFrame, y_train: np.ndarray, cat_cols,
    ...
    """
    OOF target encoding for:
      - mean(y)
      - mean(y<0)
      - mean(y==0)
      - log1p(count)
    Returns:
      te_train: (N, 4*len(cat_cols)) float32
      full_maps: dict[col] = dict with arrays/maps for applying to val/test
    """
    N = len(y_train)
    if not cat_cols:
...
def _linear_calibration(y_true: np.ndarray, pred: np.ndarray):
    """
    Fit y ~= a*pred + b (least squares).
    Keep a >= 0 to preserve ranking (gini/spearman stability).
    """
...
def _blend_and_adjust(mean_pred, huber_pred, hurdle_pred, q20_pred, pneg,
    ...
    # conservative gate near/under zero bound or high negative-risk
    gate = (q20_pred < 0) | (pneg > thr_gate)
    adj = np.where(gate, np.minimum(adj, q20_pred), adj)

    # hard cap for very high negative probability
...
def main():
    ...
    # ----------------------------
    # Choose seq_cols from TRAIN only
    # ----------------------------
    fixed_seq = [c for c in sorted(set(NUMERICAL_COLS + TCN_TEMPORAL_FEATURES)) if c in numeric_cols]
    # add additional high-variance numeric cols from TRAIN to enrich temporal subset
    extra_k = 20
    ...
    seq_cols = seq_cols[:40]  # keep compact to control feature size/runtime

    # ----------------------------
    # Build user-level bases (agg + cats); add seq features separately for fixed ordering
    # ----------------------------
    ...
    # ----------------------------
    # OOF target encoding on TRAIN users only; apply full mapping to VAL/TEST
    # ----------------------------
    ...
    # categorical nunique (no leakage)
    ...
    # ----------------------------
    # Internal split for early stopping (keep VAL purely for calibration/selection)
    # ----------------------------
    ...
    # ----------------------------
    # Train model suite (ensembled mean + quantile + classifiers + hurdle regressors)
    # ----------------------------
    seeds = [42, 202]  # small ensemble for stability
    ...
    # Quantile (single seed is typically enough)
    ...
    # Hurdle regressors: separate pos/neg magnitude models
    pos_mask = y_tr > 0
    neg_mask = y_tr < 0

    # fallbacks (if subset tiny)
    if pos_mask.sum() < 200:
        pos_model = None
    else:
    ...
    # ----------------------------
    # Calibration search on VAL (maximize Pareto score)
    # ----------------------------
                ...
                for rs in grid_risk:
                    for tg in grid_thr_gate:
                        for tc in grid_thr_cap:
                            ...
                            # very mild clip to prevent extreme outliers from inflating RMSE
    ...
    out_path = "/home/jupyter/AviaAgentMonty_1226/tasks/BT_IOS_2503_Pareto/run_deepresearch/lgbm_suite_results.json"
...
if __name__ == "__main__":
    ...
    # REQUIRED: score must be computed on TEST using compute_pareto_multi_objective()
