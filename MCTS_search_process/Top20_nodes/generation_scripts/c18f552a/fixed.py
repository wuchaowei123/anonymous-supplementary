#!/usr/bin/env python3
"""
anonymous_institutionAgentMonty - Execution Node: c18f552a
Type: fixed
Generated: 2026-01-14T01:11:53.856787
Fix Attempts: 1
...
def standardize_3d(train_X: np.ndarray, train_mask: np.ndarray, X: np.ndarray, mask: np.ndarray, eps=1e-6):
    ...
    cnt = np.clip(m.sum(axis=(0, 1)), 1.0, None)
    mu = (Xtr * m).sum(axis=(0, 1)) / cnt
...
def build_user_matrix(X3d: np.ndarray, mask: np.ndarray, X_static: np.ndarray, X_cat: np.ndarray):
    ...
    daycnt = mask.sum(axis=1).astype(np.float32)
...
def main():
    ...
    out_path = "/home/jupyter/anonymous_institutionAgentMonty_1226/tasks/BT_IOS_2503_Pareto/run_deepresearch/improved_gbdt_ensemble_results.json"
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    with open(out_path, "w") as f:
        json.dump(
            {
                "method": "Hybrid GBDT Ensemble: AWMSE mean + q20 + hurdle(sign)
    # === SAVE MODEL ===
    node_dir = "/home/jupyter/anonymous_institutionAgentMonty_1226/tasks/BT_IOS_2503_Pareto/run_20260112_102800/c18f552a"
 ...
 + conservative calibration",
    ...
    try:
        ...
        import joblib
        save_dir = Path("/home/jupyter/saved_models/c18f552a")
        ...
        with open(save_dir / "metadata.json", 'w') as f:
            json.dump({'node_id': 'c18f552a', 'saved': saved}, f)
...
# ========== CORRECTED SAVING CODE ==========
    ...
    save_dir = Path(f"/home/jupyter/saved_models_final/c18f552a")
    save_dir.mkdir(parents=True, exist_ok=True)
