#!/usr/bin/env python3
"""
anonymous_institutionAgentMonty - Execution Node: a31d2ff8
Type: original
Generated: 2026-01-13T08:22:02.930894
Mutation: replicate
Parent: None
...
def predict_model(model, X_seq_num, X_seq_cat, X_static, batch_size=2048):
    ...
    for xb_num, xb_cat, xb_static in dl:
        ...
        preds_mean.append(y_mean.detach().cpu().numpy())
        preds_q20.append(y_q20.detach().cpu().numpy())
        preds_pneg.append(p_neg.detach().cpu().numpy())
...
def main():
    try:
        ...
        model = train_physics_inspired_model(
            Xtr_num, Xtr_cat, Xtr_static, ytr,
            Xva_num, Xva_cat, Xva_static, yva,
            cat_sizes=cat_sizes if len(cat_cols) > 0 else {c: 1 for c in cat_cols},
            cat_cols=cat_cols,
            epochs=35,
            batch_size=1024,
        )
