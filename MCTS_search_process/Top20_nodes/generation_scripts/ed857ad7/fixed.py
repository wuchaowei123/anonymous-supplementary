#!/usr/bin/env python3
"""
AviaAgentMonty - Execution Node: ed857ad7
Type: fixed
Generated: 2026-01-13T06:18:32.215566
Fix Attempts: 1
...
def main():
    ...
    out_path = "/home/jupyter/AviaAgentMonty_1226/tasks/BT_IOS_2503_Pareto/run_deepresearch/method_11_results.json"
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    with open(out_path, "w") as f:
        json.dump(result, f, indent=2)
    # === SAVE MODEL ===
    node_dir = "/home/jupyter/saved_models/ed857ad7"
    os.makedirs(node_dir, exist_ok=True)
    
    # Save predictions
    try:
        predictions_df = pd.DataFrame({
            'user_id': y_test_series.index.tolist() if hasattr(y_test_series, 'index') else list(range(len(y_test_series))),
            'y_true': y_test_series.values.tolist() if hasattr(y_test_series, 'values') else list(y_test_series),
            'y_pred': pred_aligned.tolist() if hasattr(pred_aligned, 'tolist') else list(pred_aligned)
        })
