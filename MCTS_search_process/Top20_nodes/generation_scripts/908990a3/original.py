...
Type: original
Generated: 2026-01-12T16:21:11.953242
Mutation: expert
Parent: None
...
"""
#!/usr/bin/env python3
"""Method 27 (Improved): Temporal Trajectory Features + Asymmetric GBDT Stack + RL-style MLP"""
...
# ----------------------------
# Global feature config (locked from train)
# ----------------------------
DAY_COL = None
...
def _pick_numeric_cols(train_df: pd.DataFrame, max_cols: int = 80):
    ...
    # Force include the hand-picked set when present
    forced = [c for c in NUMERICAL_COLS if c in numeric]
    rest = [c for c in numeric if c not in forced]

    if len(rest) > 0:
        # variance on raw rows (train only); avoids target usage
...
def _pick_cat_cols(train_df: pd.DataFrame):
    ...
    # keep only reasonably-informative: not too many missing
    out = []
    for c in cat:
...
# ----------------------------
# Temporal feature engineering (7-day trajectory)
# ----------------------------
...
def create_features(df, is_train=True):
    """
    User-level aggregation + temporal trajectory features:
      1) Velocity: (mean(days5-7) - mean(days1-3)) / 4
      2) Acceleration: change in velocity across early vs late windows
      3) Volatility: std(daily_changes) / (abs(mean)+eps)
      4) Recency: day7/day1 ratio (robust)
      5) Peak timing: max occurs in last 2 days
      6) Rolling/cumulative stats (last3 sum/mean, nonzero-days, slope)
    """
    ...
    if SELECTED_NUM_COLS is None:
        # If called before main initializes (shouldn't), fall back to intersection.
    ...
    # --- Categorical: first/last/nunique (kept as raw for later encoding) ---
    if len(cat_cols) > 0:
    ...
    if len(num_cols) > 0:
        ...
        # Temporal features require day index
        if day_col and df_num[day_col].nunique() >= 2:
            # Build a (user x day) matrix per feature via unstack (limited to SELECTED_NUM_COLS)
            # Note: This is the main "temporal utilization" step.
            seq = df_num.set_index([ID_COL, day_col])[num_cols].unstack(day_col)

            # Ensure columns for days 1..7 exist if this is the known structure
            # If day values differ, reindex to sorted unique days (still works).
            days = sorted([d for d in seq.columns.levels[1].tolist() if pd.notna(d)])
            # Prefer 1..7 when applicable
            if all(d in days for d in [1, 2, 3, 4, 5, 6, 7]):
                days = [1, 2, 3, 4, 5, 6, 7]

            seq = seq.reindex(columns=pd.MultiIndex.from_product([num_cols, days]), copy=False)
            # Fill missing with 0 (consistent with original code's fillna(0))
            seq = seq.fillna(0.0)
            ...
            for col in num_cols:
                x = seq[col].to_numpy(dtype=np.float32, copy=False)  # shape (n_users, n_days)
                n_days = x.shape[1]
                if n_days == 0:
                    continue
                ...
                # 3) Volatility (std of daily changes / mean)
                if n_days >= 2:
                    dx = np.diff(x, axis=1)
    ...
    # Attach target at user-level for train/val
...
# ----------------------------
# Encoding categorical user-level features (train-only fit to avoid leakage)
# ----------------------------
def fit_apply_cat_encodings(train_u: pd.DataFrame, val_u: pd.DataFrame, test_u: pd.DataFrame):
    # Identify raw cat-derived cols (from create_features) still non-numeric
    cat_cols = [
    ...
    # Prior (train only) for target-encoding of "negative rate"
    y_train = out_train[TARGET_COL].values
    prior_neg = float((y_train < 0).mean())
    m = 30.0  # smoothing strength
    ...
    for c in cat_cols:
        ...
        # Negative-rate target encoding (train only)
...
# ----------------------------
# RL-style MLP (Value Network) with AWMSE-style weighting
# ----------------------------
class ValueNetwork(nn.Module):
    """Q-Network for predicting LTV given state (user features)"""
...
def train_value_network(X_train, y_train, X_val, y_val, epochs=80, batch_size=512):
    """
    Train value network with RL-inspired stability tricks + asymmetric weights.
    """
    ...
    for epoch in range(epochs):
        ...
        for X_batch, y_batch in train_loader:
            ...
            # Asymmetric weight by FP/FN (piecewise, as business cost proxy)
...
# ----------------------------
# LightGBM custom objective (AWMSE)
# ----------------------------
...
# Silent score for calibration search (does NOT modify compute_score)
...
def main():
    ...
    # Lock column selections from TRAIN only (no leakage)
    global DAY_COL, SELECTED_NUM_COLS, SELECTED_CAT_COLS
    DAY_COL = _infer_day_col(train_df)
    ...
    # Encode categorical-derived features using train-only fit
    ...
    # ----------------------------
    # Model 1: RL-style MLP (asymmetric)
    # ----------------------------
    ...
    # ----------------------------
    # Model 2: RandomForest (robust baseline)
    # ----------------------------
    ...
    # ----------------------------
    # Model 3: LightGBM (custom AWMSE objective)
    # ----------------------------
    ...
    # ----------------------------
    # Model 4: LightGBM Quantile (risk-averse lower bound proxy)
    # ----------------------------
    ...
    # ----------------------------
    # Model 5: Sign-risk classifier (P(y < 0))
    # ----------------------------
    ...
    # ----------------------------
    # Level-2 stacking (holdout meta on VAL only, avoids leakage)
    # ----------------------------
    ...
    meta_X_val = np.vstack([
        ...
        rl_val_pred,
        pneg_val,
        (aw_val_pred - q_val_pred),   # uncertainty proxy
    ]).T.astype(np.float32)
    ...
    meta_w[y_val < 0] = 1.67  # emphasize negatives (risk)
    ...
    # ----------------------------
    # Conservative calibration search (VAL only; apply to TEST)
    # We search an additive shift "delta" and a bias term to reduce budget error
    # without changing ranking much.
    # ----------------------------
    ...
    for d in deltas:
        ...
        # bias to align sums (helps error_rate); learned from val only
    ...
    # ----------------------------
    # Ground truth alignment / TEST scoring
    # ----------------------------
