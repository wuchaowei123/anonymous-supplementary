#!/usr/bin/env python3
"""
anonymous_institutionAgentMonty - Execution Node: 162c38d7
Type: original
Generated: 2026-01-13T23:35:50.299983
Mutation: None
Parent: None
...
def build_target_encoding_features_v2(Xc_tr, ytr, Xc_va, Xc_te, cat_names, seed=42):
    ...
    y_neg_val = np.clip(ytr, None, 0).astype(np.float32)  # negative or 0
    ...
    for j, cname in enumerate(cat_names):
        ...
        # expected signed parts + magnitude proxy
...
def build_cat_count_features(train_df: pd.DataFrame, dfs, cat_cols):
    ...
    # counts on train only (no leakage from val/test)
    counts = {}
    for c in cat_cols:
...
def _common_lgb_params(seed: int):
    return dict(
        ...
        extra_trees=True,  # often helps generalization on tabular
    )

def train_lgb_reg_aw(Xtr, ytr, Xva, yva, cat_idx, seed=42):
    # IMPORTANT FIX: actually use the custom asymmetric objective during training
...
def train_lgb_quantile(Xtr, ytr, Xva, yva, cat_idx, alpha=0.20, seed=42):
    ...
    params.update(dict(
        ...
        extra_trees=False,  # quantile + extra_trees can be unstable
    ))
    dtr = lgb.Dataset(Xtr, label=ytr, categorical_feature=cat_idx, free_raw_data=False)
    dva = lgb.Dataset(Xva, label=yva, categorical_feature=cat_idx, free_raw_data=False)
    model = lgb.train(
...
def apply_policy_v2(pred_aw, pred_huber, pred_q20, pred_q80, pred_moe, pneg, pzero,
    ...
    if risk_scale > 0:
        # dampen positive part proportional to negative-risk and uncertainty gap
    ...
    # slight stabilization: if q80 way below 0 but y is positive, be conservative
...
def tune_on_val_v2(yva, pred_aw_va, pred_huber_va, pred_q20_va, pred_q80_va, pred_moe_va, pneg_va, pzero_va, anchor_mean: float):
    # compact grid; focus on stability + mean calibration
                        ...
                        for w_q in w_q_grid:
                            for w_aw in w_aw_grid:
                                for w_huber in w_huber_grid:
                                    ...
                                    # IMPORTANT: mean-anchor correction (doesn't affect rank metrics, helps error_rate stability)
...
def main():
    ...
    # Anchor mean estimate (train+val) for mean-correction on final predictions (NO test labels used)
    ...
    # numeric tensor + richer temporal features
    ...
    # categorical indices: last block of cat codes
    ...
    # robust clipping bounds (train only)
    ...
    # Train base models
    # ===========================
    print("\n[Training] AWMSE regressors (custom objective)...")
    ...
    # clip predictions to train robust range
    ...
    # Mean anchoring (key for error_rate robustness on test)
    final_pred = mean_correct(final_pred, anchor_mean=anchor_mean)

    # align to test user index
    ...
    out_path = "/home/jupyter/anonymous_institutionAgentMonty_1226/tasks/BT_IOS_2503_Pareto/run_deepresearch/improved_lgbm_temporal_results_v2.json"
