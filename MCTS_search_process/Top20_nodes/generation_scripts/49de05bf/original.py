#!/usr/bin/env python3
"""
anonymous_institutionAgentMonty - Execution Node: 49de05bf
Type: original
Generated: 2026-01-13T11:25:17.610422
Mutation: None
Parent: None
...
# ===========================
# ⚠️ SCORING FUNCTIONS (DO NOT MODIFY) - PRESERVED EXACTLY
# ===========================
...
# ===========================
# Helpers (silent score for tuning to avoid massive prints)
# ===========================
...
def train_physics_inspired_model(
    ...
    for epoch in range(1, epochs + 1):
        ...
        if do_eval:
            ...
            # evaluation calibration kept mild; final tuning happens later
...
# ===========================
# LightGBM: features + models
# ===========================
def build_gbdt_features_from_seq(X_seq_num, X_static, X_cat, n_base_num: int):
    """
    Use scaled values + derived dynamics (abs diff, accel) + selected day snapshots.
    Keeps feature count moderate but expressive.
    """
    ...
    # ratios/trends (stable on scaled space)
    trend_73 = day7 - day3
    trend_31 = day3 - day1
...
def target_encode_oof(train_cats: pd.DataFrame, y: np.ndarray, other_cats: list, smoothing: float = 50.0, n_splits: int = 5, seed: int = 42):
    """
    OOF target encoding to avoid leakage. Produces two encodings per cat:
      - smoothed mean(y)
      - smoothed rate(y<0)
    """
    ...
    for tr_idx, va_idx in kf.split(train_cats):
        ...
        for j, c in enumerate(cols):
            grp = tr.groupby(c).agg(
                cnt=(c, "size"),
            )
            # compute sums via align on index
            # mean
    ...
    def encode_full(df_cats: pd.DataFrame):
        ...
        for j, c in enumerate(cols):
            tr = train_cats
            grp = tr.groupby(c).agg(cnt=(c, "size"))
...
def train_lightgbm_suite(Xtr, ytr, Xva, yva, seed=42):
    ...
    params_aw["objective"] = lgb_awmse_obj
    aw = lgb.train(
        params_aw, dtr,
    ...
    # L2 regressor (often better for rank/correlation)
    params_l2 = {
    ...
    # Negative classifier for p(ltv < 0)
    ...
    spw = (neg / max(1.0, pos)) * 1.67  # emphasize catching negatives
    params_c = {
        "objective": "binary",
        "metric": "auc",
        "learning_rate": 0.04,
...
def fit_affine_positive_slope(y_true, y_pred):
    """
    Fit y ≈ a * pred + b on val (least squares).
    Enforce a >= 0 (keep ranks).
    Then adjust bias to match sums on val (helps error_rate).
    """
    ...
    if a < 0:
        a = 0.0
    # sum-match adjustment (constant shift)
...
def tune_ensemble_on_val(y_val, deep_pack, lgb_pack):
    ...
    # small but effective search space (no print spam)
    alphas = [0.0, 0.10, 0.20, 0.30, 0.40]
    betas = [0.0, 0.25, 0.50, 0.75, 1.00]
    deltas = [-240, -200, -160, -120, -80, -40, -20, 0, 20, 40, 80, 120, 160, 200]
    ws = [0.0, 0.25, 0.50, 0.75, 1.00]     # deep weight
    vs = [0.0, 0.50, 1.00]                 # within-lgb: awmse weight vs l2

    best = None
    for a in alphas:
        for b in betas:
            # precompute calibrated components for given (a,b) so only delta/weights loop remains
            deep_cal0 = apply_conservative_calibration(deep_mean, deep_q20, deep_pneg, alpha=a, beta=b, delta=0.0)
            lgb_mix_mean0 = None  # depends on v
            lgb_cal0_by_v = {}
            ...
            for d in deltas:
                # delta is just a constant shift (since apply_conservative_calibration subtracts delta)
                ...
                for v in vs:
                    ...
                    for w in ws:
                        ...
                        key = (sc, m["spearman"], -m["error_rate"])  # prefer score; tie-break correlation and error-rate
...
def main():
    try:
        ...
        # Build GBDT features from the same deep-preprocessed sequences (scaled) + OOF target encoding
        ...
        # Tune ensemble + affine calibration on VAL (no test leakage)
        ...
        # Align to test user index and score
